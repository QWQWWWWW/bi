	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0","margin":"0px 0 0","borderRadius":"0px","background":"none","width":"calc(100% - 400px)","float":"left","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"31.3%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"480px"}' class="item">
			<img @click.stop="imgPreView(item.zuoyetupian)" v-if="item.zuoyetupian && item.zuoyetupian.substr(0,4)=='http'" :src="item.zuoyetupian.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.zuoyetupian?item.zuoyetupian.split(',')[0]:''))" v-else :src="baseUrl + (item.zuoyetupian?item.zuoyetupian.split(',')[0]:'')" class="image" />
			<div class="content">
		      <div :style='{"padding":"30% 10px 20px 0","borderColor":"#ddd","color":"inherit","alignItems":"center","display":"block","textIndent":"2em","overflow":"hidden","borderWidth":"0 1px 0 0","width":"48%","lineHeight":"30px","fontSize":"inherit","borderStyle":"dashed","height":"100%"}'>{{item.zuoyeyaoqiu}}</div>
