...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="分析报告" prop="fenxibaogao">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="分析报告"
              v-model="ruleForm.fenxibaogao">
            </el-input>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='fenxibaogao'){
              this.ruleForm.fenxibaogao = obj[o];
              this.ro.fenxibaogao = true;
              continue;
            }
          }
        }else if(type=='edit'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`xuexifenxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('xuexifenxi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`xuexifenxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
