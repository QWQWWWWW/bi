...
<div class="content" :style='{"minHeight":"100vh","width":"100%","padding":"30px","background":"url(http://codegen.caihongy.cn/20240320/095a0a9d9da74a8fa6c854b2144c65aa.png) no-repeat center top / cover","height":"auto"}'>
	...
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":" 1px solid rgba(167, 180, 201,.3)    ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('zuoyechengji','首页总数')">
			...
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex","order":"1"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","fontWeight":"bold","height":"24px"}'>{{zuoyechengjiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>作业成绩总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgba(167, 180, 201,.3)  ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('banjixinxi','首页总数')">
			<div :style='{"alignItems":"center","background":"#00e682","display":"flex","width":"80px","justifyContent":"center","height":"80px","order":"2"}'>
				<span class="icon iconfont icon-xihuan" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#374254","fontWeight":"bold","height":"24px"}'>{{banjixinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>班级信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="zuoyechengjiChart1" class="echarts1" v-if="isAuth('zuoyechengji','首页统计')"></div>
		<div id="banjixinxiChart1" class="echarts2" v-if="isAuth('banjixinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getbanjixinxiCount();
		this.banjixinxiChat1();
	},
	methods:{
		// 词云
		...
		getbanjixinxiCount() {
			this.$http({
				url: `banjixinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.banjixinxiCount = data.data
				}
			})
		},
		banjixinxiChat1() {
        ...
        var banjixinxiChart1 = echarts.init(document.getElementById("banjixinxiChart1"),'macarons');
        this.$http({
            url: `banjixinxi/value/banjimingcheng/banjirenshu`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].banjimingcheng);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].banjimingcheng
                    })
                }
                var option = {};
                ...
                // 使用刚指定的配置项和数据显示图表。
                banjixinxiChart1.setOption(option);
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, banjixinxiChart1)
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    banjixinxiChart1.resize();
                };
            }
        });
      })
